...
static void tearDown(void) {
    /* Nothing to free - pools are stack/static allocated */
}
...
void test_MemoryPools_InitializeCorrectly(void) {
    ...
    TEST_ASSERT_EQUAL(0, stats.hash_allocations);
    TEST_ASSERT_EQUAL(0, stats.hash_peak_usage);
    TEST_ASSERT_EQUAL(0, stats.hash_failures);
...
void test_MemoryPools_OrdersAllocateFromPool(void) {
    ...
    /* Should have allocated hash entries for order lookup */
    TEST_ASSERT_TRUE(stats.hash_allocations >= 3);
    TEST_ASSERT_EQUAL(0, stats.hash_failures);
...
void test_MemoryPools_FlushReturnsAllToPool(void) {
    ...
    for (uint32_t i = 0; i < 10; i++) {
        new_order_msg_t order = {1, "TEST", 100 + i, 50, SIDE_BUY, i + 1};
        output_buffer_init(&output);
        order_book_add_order(&book, &order, 1, &output);
    }
    ...
    for (uint32_t i = 0; i < 10; i++) {
        new_order_msg_t order = {1, "TEST", 200 + i, 50, SIDE_SELL, i + 100};
        output_buffer_init(&output);
        order_book_add_order(&book, &order, 1, &output);
    }
...
void test_MemoryPools_TradeRemovesOrders(void) {
    ...
    memory_pools_get_stats(&pools, &stats_after_trade);
    ...
    for (uint32_t i = 0; i < 5; i++) {
        new_order_msg_t order = {1, "TEST", 150 + i, 50, SIDE_BUY, i + 10};
        output_buffer_init(&output);
        order_book_add_order(&book, &order, 1, &output);
    }
    
    memory_pool_stats_t stats_after_reuse;
    memory_pools_get_stats(&pools, &stats_after_reuse);
...
void test_MemoryPools_PeakUsageTracking(void) {
    ...
    memory_pools_get_stats(&pools, &stats1);
    ...
    memory_pools_get_stats(&pools, &stats2);
    ...
    memory_pools_get_stats(&pools, &stats3);
...
void test_MemoryPools_SharedByMultipleBooks(void) {
    ...
    new_order_msg_t ibm_order = {1, "IBM", 100, 50, SIDE_BUY, 1};
    output_buffer_init(&output);
    order_book_add_order(&book1, &ibm_order, 1, &output);
    
    new_order_msg_t aapl_order = {2, "AAPL", 150, 30, SIDE_SELL, 1};
    output_buffer_init(&output);
...
void test_MemoryPools_DataIntegrityUnderReuse(void) {
    ...
    new_order_msg_t buy2 = {2, "TEST", 200, 75, SIDE_BUY, 2};
    output_buffer_init(&output);
