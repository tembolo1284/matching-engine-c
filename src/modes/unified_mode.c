...
static bool init_queues(unified_server_t* server) {
    /*
     * IMPORTANT: Use aligned_alloc_64() instead of malloc() for queues.
     * The lockfree queue structures contain _Alignas(64) members for
     * cache-line alignment. GCC 15+ enforces this strictly, and using
     * malloc() (which only guarantees 16-byte alignment) causes crashes
     * when accessing atomic operations on misaligned memory.
     */
    server->input_queue_0 = aligned_alloc_64(sizeof(input_envelope_queue_t));
    server->output_queue_0 = aligned_alloc_64(sizeof(output_envelope_queue_t));
    ...
    if (!server->config.single_processor) {
        server->input_queue_1 = aligned_alloc_64(sizeof(input_envelope_queue_t));
        server->output_queue_1 = aligned_alloc_64(sizeof(output_envelope_queue_t));
