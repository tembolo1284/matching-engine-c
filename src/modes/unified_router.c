...
/* Global quiet mode flag - set by router thread from server config */
static bool g_router_quiet = false;
...
void unified_broadcast_to_all(unified_server_t* server, const output_msg_t* msg) {
    ...
    if (elapsed_ns > 1000000ULL && !g_router_quiet) {  /* > 1ms */
...
static void process_output_envelope(unified_server_t* server,
    ...
    } else if (elapsed_ns > 1000000ULL) {
        /* Still count slow envelopes even in quiet mode */
        dbg_envelopes_slow++;
    }
...
void* unified_output_router_thread(void* arg) {
    ...
    /* Set global quiet flag from server config */
    g_router_quiet = server->config.quiet_mode;
    ...
    while (!atomic_load(&g_shutdown)) {
        ...
        /* DEBUG: Periodic stats every 2 seconds (only if not quiet) */
        uint64_t now = debug_now_ns();
        if (!g_router_quiet && now - last_debug_time >= DEBUG_INTERVAL_NS) {
            ...
            idle_spins = 0;
        } else if (g_router_quiet) {
            /* In quiet mode, just update tracking vars without printing */
    ...
    /* DEBUG: Final summary (only if not quiet) */
    if (!g_router_quiet) {
