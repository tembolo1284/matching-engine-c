...
static bool send_multicast_message(multicast_publisher_context_t* ctx,
    ...
    char buffer[MAX_MULTICAST_PACKET_SIZE];
    size_t msg_len = 0;
    
    if (ctx->config.use_binary_output) {
        // Binary protocol
        msg_len = binary_message_formatter_format(msg, buffer, sizeof(buffer));
        if (msg_len == 0) {
        ...
        // CSV protocol
        message_formatter_format(msg, buffer, sizeof(buffer));
        msg_len = strlen(buffer);
        
        // Add newline if not present
        if (msg_len > 0 && buffer[msg_len - 1] != '\n') {
            if (msg_len + 1 < sizeof(buffer)) {
                buffer[msg_len] = '\n';
                msg_len++;
                buffer[msg_len] = '\0';
            }
    ...
    ssize_t sent = sendto(ctx->sockfd, buffer, msg_len, 0,
    ...
    while (!atomic_load(ctx->shutdown_flag)) {
        ...
        for (int q = 0; q < ctx->num_input_queues; q++) {
            ...
            for (size_t i = 0; i < count; i++) {
                ...
                send_multicast_message(ctx, &envelope->msg);
