...
static void drain_until_trades(engine_client_t* client,
    ...
    uint32_t drain_recv_calls = 0;
    uint32_t drain_msgs_total = 0;
    ...
    if (!quiet) {
        ...
        fprintf(stderr, "[DBG-DRAIN] Phase 1 done: %.1fs | gained %u trades | "
                "total msgs: %u | iters: %u | has_data: %d\n",
